...
if ($this->ms['show_main'])
{
	...
	$colspan=4;
	$str="SELECT *,d.name from tx_multishop_payment_methods p, tx_multishop_payment_methods_description d where d.language_id='".$this->sys_language_uid."' and p.id=d.id order by p.sort_order";
	$qry=$GLOBALS['TYPO3_DB']->sql_query($str);
	$tr_type='even';
	if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry) > 0)
	{
