...
if ($_REQUEST['sub']=='add_shipping_method' and $_REQUEST['shipping_method_code'])
{
	...
	if ($erno or !$this->post)
	{
		...
		$tmpcontent.='<form action="'.mslib_fe::typolink(',2003','&tx_multishop_pi1[page_section]='.$this->ms['page']).'" id="add_shipping_form" method="post">';
...
if ($this->ms['show_main'])
{
	...
	$colspan=4;
	$str="SELECT *,d.name from tx_multishop_shipping_methods p, tx_multishop_shipping_methods_description d where d.language_id='".$this->sys_language_uid."' and p.id=d.id order by p.sort_order";	
	$qry=$GLOBALS['TYPO3_DB']->sql_query($str);
	$tr_type='even';
	if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry) > 0)
	{
	...
	else $tmpcontent.=$this->pi_getLL('currently_there_are_no_shipping_methods_defined').'.';
	...
	foreach ($shipping_methods as $code => $item)
	{
		...
		if ($item['image']) $tmpcontent.='<span class="multishop_psp_image_wrapper"><span class="multishop_psp_image"><img src="'.$this->FULL_HTTP_URL_MS.'templates/images/shipping/'.$item['image'].'" alt="Add '.htmlspecialchars($item['name']).'"></span></span>';
		else				$tmpcontent.=$item['name'];
		$tmpcontent.='</a></div></li>';
