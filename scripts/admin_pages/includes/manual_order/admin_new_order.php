...
else
{
	...
	{	
		...
		{
			if ($customer['email']) $content.='<option value="'.$customer['uid'].'">'.htmlspecialchars($customer['name'].' ('.$customer['email'].')').'</option>';
		}
		$content.='</select>			
';
		$content.='
...
';
		...
		<div class="account-field">
			...
			<input type="text" name="tx_multishop_pi1[telephone]" id="telephone" class="telephone" value="'.htmlspecialchars($user['telephone']).'"><span class="error-space"></span>
...
';
	...
	if ($tmpcontent_con)
		...
		';
		$content.='<div id="delivery_address_category" class="hide"><div class="main-heading"><h2>'.$this->pi_getLL('delivery_address').'</h2></div>'.$tmpcontent.'
		<script>
