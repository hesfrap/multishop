...
if ($this->post and $_REQUEST['action']=='edit_cms')
{
	if ($this->post['cms_id'])
	{
		// update
		$array=array();		
		if (!$this->post['tx_multishop_pi1']['type'] and $this->post['tx_multishop_pi1']['custom_type']) $array['type']=$this->post['tx_multishop_pi1']['custom_type'];
		else $array['type']					=	$this->post['tx_multishop_pi1']['type'];
...
if ($cms['id'] or $_REQUEST['action']=='edit_cms')
{
	...
	if (is_array($payment_methods)) foreach ($payment_methods as $key => $value)	$types['email_order_confirmation_'.$key]	=htmlspecialchars($this->pi_getLL('email_order_confirmation_letter')).' ('.$key.')';
	$types['email_order_paid_letter']			=	htmlspecialchars($this->pi_getLL('email_order_paid_letter'));
	if (is_array($payment_methods)) foreach ($payment_methods as $key => $value)	$types['email_order_paid_letter_'.$key]	=htmlspecialchars($this->pi_getLL('email_order_paid_letter')).' ('.$key.')';
	...
	if (is_array($payment_methods)) foreach ($payment_methods as $key => $value)	$types['order_received_thank_you_page_'.$key]	=htmlspecialchars($this->pi_getLL('checkout_finished_page')).' ('.$key.')';
