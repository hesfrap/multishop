...
if ($_REQUEST['section']=='edit' or $_REQUEST['section']=='add')
{
	if ($this->post)
	{
		...
		elseif (!$this->post['feed_type'] and (!is_array($this->post['fields']) || !count($this->post['fields'])))
		{
...
if (is_array($this->post['fields']) and count($this->post['fields']))
{
	foreach ($this->post['fields'] as $field)
	{
		...
		if ($field=='custom_field') {
			$content.='<div class="account-field"><label><input name="fields_headers['.$counter.']" type="text" value="'.$this->post['fields_headers'][$counter].'" /></label><span><input name="fields_values['.$counter.']" type="text" value="'.$this->post['fields_values'][$counter].'" /></span></div>';
		}
		$content.='
		</div>';	
	}
...
$content.='
	...
	$content.='</select><input class="delete_field msadmin_button" name="delete_field" type="button" value="'.htmlspecialchars($this->pi_getLL('delete')).'" /></div></div>\';
		...
		jQuery(".msAdminProductsFeedSelectField").live(\'change\', function()
		{
			...
			{
				$(this).next().remove();
				$(this).parent().append(\'<div class="account-field"><label><input name="fields_headers[\'+counter+\']" type="text" /></label><span><input name="fields_values[\'+counter+\']" type="text" /></span></div>\');
			}
		});
