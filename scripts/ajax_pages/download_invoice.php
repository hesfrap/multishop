...
if ($invoice['orders_id'])
{
	...
	if (!$name) $name=$order['billing_name'];
	...
	if($order['payment_method_label']) $pdfdata['address'][$this->pi_getLL('payment_method')]=$order['payment_method_label'];
	if ($order['billing_vat_id']) $pdfdata['address'][$this->pi_getLL('vat_id')]=$order['billing_vat_id'];
	// top right area eof
	// order details	
...
//					$price=round(($options['options_values_price']*($product['products_tax']/100))+$options['options_values_price'],4);
		...
		$pdfdata['data'][]=array($prefix.number_format($product['qty']),$product['products_model'],$product['products_name'].$sub_content,number_format($product['products_tax'])."%",mslib_fe::Money2PDFDutchString($prefix.$final_price),mslib_fe::Money2PDFDutchString($prefix.($product['qty']*$final_price)).$sub_prices);
	}
	// order details eof	
	// total
...
//			$this->Cell('52',6,'--------------------------------','',0,'R');		
			...
			{
				$counter++;
				if ($counter==$total) 	$this->SetFont('','B');
				else				$this->SetFont('','');
...
//				$this->Rect($x, $y, $w, $h);
	...
	{	
		if ($pdfdata['shipping_method_costs']) 	$array[]=array(utf8_decode($this->pi_getLL('shipping_costs')),$pdfdata['shipping_method_costs']);
		if ($pdfdata['payment_method_costs']) 	$array[]=array(utf8_decode($this->pi_getLL('payment_costs')),$pdfdata['payment_method_costs']);	
	...
	if ($pdfdata['discount'])				$array[]=array(utf8_decode($this->pi_getLL('discount')),$pdfdata['discount']);	
