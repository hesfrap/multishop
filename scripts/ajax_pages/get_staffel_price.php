...
if ($this->ADMIN_USER) {
	$product=mslib_fe::getProduct($pid, '', '', 1);
} else {
	$product=mslib_fe::getProduct($pid);
}
		...
		if ($continue) {
			...
			if (count($array)) {
				...
				foreach ($array as $item) {
					$str="SELECT * from tx_multishop_products_attributes a, tx_multishop_products_options o, tx_multishop_products_options_values ov where a.products_id='".$pid."' and a.options_id='".$key."' and a.options_values_id='".$item."' and o.hide_in_cart=0 and a.options_id=o.products_options_id and o.language_id='".$this->sys_language_uid."' and ov.language_id='".$this->sys_language_uid."' and a.options_values_id=ov.products_options_values_id";
					$qry=$GLOBALS['TYPO3_DB']->sql_query($str);
