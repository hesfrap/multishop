...
if (mslib_fe::loggedin()) {
	...
	} else if($this->ADMIN_USER) {
		if (!$this->conf['fe_cmsadmin_usergroup']) $this->conf['fe_cmsadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_customersadmin_usergroup']) $this->conf['fe_customersadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_catalogadmin_usergroup']) $this->conf['fe_catalogadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_ordersadmin_usergroup']) $this->conf['fe_ordersadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_storesadmin_usergroup']) $this->conf['fe_storesadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_cmsadmin_usergroup']) $this->conf['fe_cmsadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_searchadmin_usergroup']) $this->conf['fe_searchadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_systemadmin_usergroup']) $this->conf['fe_systemadmin_usergroup']=$this->conf['fe_admin_usergroup'];
		if (!$this->conf['fe_statisticsadmin_usergroup']) $this->conf['fe_statisticsadmin_usergroup']=$this->conf['fe_admin_usergroup'];
...
if ($this->conf['fe_customer_usergroup']) 			$this->excluded_userGroups[]=$this->conf['fe_customer_usergroup'];
if ($this->conf['fe_admin_usergroup']) 				$this->excluded_userGroups[]=$this->conf['fe_admin_usergroup'];
if ($this->conf['fe_rootadmin_usergroup']) 			$this->excluded_userGroups[]=$this->conf['fe_rootadmin_usergroup'];
if ($this->conf['fe_cmsadmin_usergroup']) 			$this->excluded_userGroups[]=$this->conf['fe_cmsadmin_usergroup'];
if ($this->conf['fe_customersadmin_usergroup']) 	$this->excluded_userGroups[]=$this->conf['fe_customersadmin_usergroup'];
if ($this->conf['fe_catalogadmin_usergroup']) 		$this->excluded_userGroups[]=$this->conf['fe_catalogadmin_usergroup'];
if ($this->conf['fe_ordersadmin_usergroup']) 		$this->excluded_userGroups[]=$this->conf['fe_ordersadmin_usergroup'];
...
if ($this->ADMIN_USER)
{
	...
	{
		if ((count($enabled_countries) and (in_array($language['lg_iso_2'],$enabled_countries))) or (!count($enabled_countries)))	$this->languages[$language['uid']]=$language;
	}
...
if ($this->disableMetatags) $this->conf['disableMetatags']=1;
// TypoScript custom settings
if ($this->conf['customSettings']=='{$plugin.multishop.customSettings}') $this->conf['customSettings']='';
if ($this->conf['customSettings'])
{
	if ($this->customSettings) $this->customSettings.="\n";
	$this->customSettings.=$this->conf['customSettings'];
}
//mslib_befe::print_r($this->conf);
...
if (is_numeric($this->conf['categoriesStartingPoint']))	$this->categoriesStartingPoint=$this->conf['categoriesStartingPoint'];
if (!is_numeric($this->categoriesStartingPoint))
{
	$this->categoriesStartingPoint=0;
}
if ($this->categoriesStartingPoint and !$this->get['categories_id']) $this->get['categories_id']=$this->categoriesStartingPoint;
...
if (!$this->masterShop and $this->conf['masterShop']) $this->masterShop=$this->conf['masterShop'];
if (!$this->showCatalogFromPage and $this->conf['catalog_shop_pid']) $this->showCatalogFromPage=$this->conf['catalog_shop_pid'];
elseif (!$this->showCatalogFromPage)	$this->showCatalogFromPage=$this->shop_pid;
$this->imageWidth 			= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'imageWidth', 's_specials');
if (!$this->custom_script) $this->custom_script		= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'custom_script', 's_misc');
if ($this->conf['custom_script']) $this->custom_script=$this->conf['custom_script'];
if ($this->custom_script)
{
	if (preg_match("/\.php$/",$this->custom_script))  $this->custom_script=preg_replace("/\.php$/","",$this->custom_script);
	if ($this->custom_script)
	{
		if (strstr($this->custom_script,"..")) die('error in custom script path: ('.htmlspecialchars($this->custom_script).')');
		else 
		{
...
if (!$this->conf['admin_template_folder']) 						$this->conf['admin_template_folder']					='admin_multishop';
...
if (!$this->conf['highslide_folder']) 							$this->conf['highslide_folder']							='highslide';
if (!$this->conf['admin_help_url']) 							$this->conf['admin_help_url']							='http://www.typo3multishop.com';
if (!$this->conf['admin_development_company_url']) 				$this->conf['admin_development_company_url']			='http://www.typo3multishop.com';
if (!$this->conf['admin_development_company_name']) 			$this->conf['admin_development_company_name']			='TYPO3 Multishop';
if (!$this->conf['admin_development_company_logo']) 	$this->conf['admin_development_company_logo']	=t3lib_extMgm::siteRelPath($this->extKey).'templates/admin_multishop/images/admin_logo.gif';
if (!$this->conf['admin_development_company_logo_gray_path']) 	$this->conf['admin_development_company_logo_gray_path']	=t3lib_extMgm::siteRelPath($this->extKey).'templates/images/powered_by_typo3multishop_gray.png';
$key='';
if ($this->ms['MODULES']['CART_PAGE_UID']) $key='_'.$this->ms['MODULES']['CART_PAGE_UID'];		
