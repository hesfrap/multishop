...
/*			
			$ors=array();
			foreach ($rand_keys as $pid)
			{
				$ors[]=$tbl.'products_id = ' . $pid;
			}
			$filter[]='('.implode(" OR ",$ors).')';
*/			
			$filter[]=$tbl.'products_id IN ('.implode(",",$rand_keys).')';
			if (is_numeric($this->get['products_id'])) $filter[]=$tbl.'products_id NOT IN ('.$this->get['products_id'].')';
		}
