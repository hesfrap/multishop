...
if ($this->ms['MODULES']['CACHE_FRONT_END'] and !$this->ms['MODULES']['CACHE_TIME_OUT_PRODUCTS_DETAIL_PAGES']) $this->ms['MODULES']['CACHE_FRONT_END']=0;
...
{
	if (strstr($this->ms['MODULES']['PRODUCTS_DETAIL_TYPE'],"/"))	require($this->DOCUMENT_ROOT.$this->ms['MODULES']['PRODUCTS_DETAIL_TYPE'].'.php');	
	elseif($this->ms['MODULES']['PRODUCTS_DETAIL_TYPE'])	require(t3lib_extMgm::extPath('multishop').'scripts/front_pages/includes/products_detail/'.$this->ms['MODULES']['PRODUCTS_DETAIL_TYPE'].'.php');		
	else require_once('includes/products_detail/default.php');
	if ($this->ms['MODULES']['CACHE_FRONT_END'])
	{
...
if (is_array($output_array['meta']) and count($output_array['meta'])) $GLOBALS['TSFE']->additionalHeaderData=array_merge($GLOBALS['TSFE']->additionalHeaderData,$output_array['meta']);
unset($output_array);	
?>