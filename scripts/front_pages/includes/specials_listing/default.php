...
$teller=0;
$specials_items='';
if (!$this->hideHeader) $content.='<div class="main-heading"><h2>'.$this->pi_getLL('specials').'</h2></div>';		
foreach ($products as $product)
{
	if ($product['products_image']) 	$image=mslib_befe::getImagePath($product['products_image'],'products',$this->imageWidth);
	else								$image='';	
	$teller++;		
	...
	if (!$this->ajax_content)
	{
		...
		if ($teller < $total) $specials_items.=',';
		$specials_items.="\n";
	}
	else
	{
