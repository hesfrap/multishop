...
else
{
	...
	{		
		...
		if ($this->ms['description']) $output_array['meta']['description'] = '<meta name="description" content="'.htmlspecialchars($this->ms['description']).'" />';	
		if ($product['products_meta_keywords']) $output_array['meta']['keywords'] 	= '<meta name="keywords" content="'.htmlspecialchars($product['products_meta_keywords']).'" />';		
		// meta tags eof	
	}
	...
	if ($this->ms['MODULES']['PRODUCTS_DETAIL_PAGE_PAGINATION'])
	{
		...
		if ($pagination_items['previous_item']) $pagination.='<div class="pagination_previous"><a href="'.$pagination_items['previous_item'].'">'.$this->pi_getLL('previous').'</a></div>';
		else											$pagination.='<div class="pagination_previous_disabled">'.$this->pi_getLL('previous').'</div>';		
		if ($pagination_items['next_item']) 	$pagination.='<div class="pagination_next"><a href="'.$pagination_items['next_item'].'">'.$this->pi_getLL('next').'</a></div>';
		else											$pagination.='<div class="pagination_next_disabled">'.$this->pi_getLL('next').'</div>';
		$pagination.='</div>';
	...
	if ($product['tax_id'] and $this->ms['MODULES']['SHOW_PRICES_WITH_AND_WITHOUT_VAT'])
	{
		...
		if ($tax)
		{
			if ($product['staffel_price'] >0) 	$price_excl_vat=(mslib_fe::calculateStaffelPrice($product['staffel_price'],$qty)/$qty);
			else								$price_excl_vat=$product['final_price'];
	...
	if ($product['products_price'] <> $product['final_price'])
	{
		...
		if ($old_price) $output['products_price']		.='<div class="old_price">'.mslib_fe::amount2Cents($old_price).'</div>';
	...
	if ($this->ms['MODULES']['SHOW_STOCK_LEVEL_AS_BOOLEAN'] != 'no')
	{
		...
		{
			case 'yes_with_image':
				if ($product['products_quantity'])  $product['products_quantity']='Voorraad: <img src="'.t3lib_extMgm::siteRelPath($this->extKey).'templates/images/icons/status_green.png" alt="'.htmlspecialchars($this->pi_getLL('in_stock')).'" />';
				else        $product['products_quantity']='Voorraad: <img src="'.t3lib_extMgm::siteRelPath($this->extKey).'templates/images/icons/status_red.png" alt="'.htmlspecialchars($this->pi_getLL('not_in_stock')).'" />';
			break;
