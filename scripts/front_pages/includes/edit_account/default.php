...
if($this->post){
	...
	// billing details eof	
	// delivery details	
	if ($this->post['delivery_first_name']) $this->post['different_delivery_address']=1;
...
else
{
	...
	{
		$tmpcontent_con.='<option value="'.t3lib_div::strtolower($country['cn_short_en']).'" '.((t3lib_div::strtolower($user['country'])==t3lib_div::strtolower($country['cn_short_en']))?'selected':'').'>'.htmlspecialchars($country['cn_short_en']).'</option>';
		$tmpcontent_con_delivery.='<option value="'.t3lib_div::strtolower($country['cn_short_en']).'" '.((t3lib_div::strtolower($user['delivery_country'])==t3lib_div::strtolower($country['cn_short_en']))?'selected':'').'>'.htmlspecialchars($country['cn_short_en']).'</option>';
	}
	if ($tmpcontent_con)
	{
		$content.='
...
<div id="input-daddress" class="account-field">
		...
		<input  type="text" name="delivery_street_name" id="delivery_address" class="delivery_address left-this" value="'.htmlspecialchars($user['delivery_street_name']).'"><span  class="error-space left-this"></span>
</div>
...
<div class="msFrontUserNameDetails">
	...
	<div id="input-password" class="account-field">
		...
		<input type="password" name="repassword" class="repassword" id="repassword" value="" /><span class="error-space"></span>	
	</div>	
</div>
